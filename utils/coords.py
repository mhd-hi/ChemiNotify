...
def is_pixel_color_match(window, coords, element_name="element", expected_colors=None, max_attempts=3, retry_delay=0.5, logger=None, tolerance=20):
    """
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            is_match = any(
                abs(pixel_color[0] - color[0]) <= tolerance and
                abs(pixel_color[1] - color[1]) <= tolerance and
                abs(pixel_color[2] - color[2]) <= tolerance
                for color in expected_colors
            )

            if logger:
                logger.debug(f"Pixel at {coords} color: {pixel_color}, Expected one of: {expected_colors}")

            if is_match:
                if logger:
                    logger.debug(f"Pixel color matches one of the expected colors: {pixel_color}")
                return True
            else:
                if logger:
                    logger.warning(
                ...
                if attempt == max_attempts and logger:
                    try:                
                        img = screenshot(region=(window.left, window.top, window.width, window.height))
                        if img:
                            save_file(
                                img, 
                        ...
                        logger.error(f"Failed to save debug screenshot: {screenshot_err}")
                
                if attempt < max_attempts:
                    time.sleep(retry_delay)
        except Exception as e:
            if logger:
                logger.error(f"Error checking pixel color (attempt {attempt}/{max_attempts}): {e}")
            if attempt < max_attempts:
                time.sleep(retry_delay)
            else:
                return False
    if logger:
        logger.error(f"Pixel at {coords} for '{element_name}' did not match expected colors after {max_attempts} attempts.")
    return False