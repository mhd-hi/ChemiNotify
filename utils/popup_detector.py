...
class PopupDetector:
        ...
        if new_title:
            ...
            try:
                ...
                if not popup_windows:
                    self.logger.warning(f"Found popup title '{new_title}' but couldn't get window object")
                ...
                if handle_automatically:
                    ...
                    self.logger.debug("Automatic handling disabled, but close_after=True")
                ...
                self.logger.warning(f"Found popup title '{new_title}' but window disappeared")
                return new_title, None, None
    ...
    def capture_popup_text(self, popup_window: gw.Window) -> Optional[str]:
        ...
        try:
            ...
            if trimmed_text:
                ...
                self.logger.debug(f"OCR text sample: '{trimmed_text[:50]}{'...' if len(trimmed_text) > 50 else ''}'")
                return trimmed_text
            else:
            ...
            if title_match and text_match:
                ...
                try:
                    if popup_type.get('action') == 'click_ok':
                        self.logger.debug(f"Handling {return_value} popup with OK button click")
                        ...
                        self.logger.debug(f"Handling {return_value} popup with standard close")
                        self.close_popup(popup_window)
                except Exception as e:
    ...
    def close_popup(self, popup_window):
        ...
        self.logger.debug(f"Attempting to close popup window: {popup_window.title if hasattr(popup_window, 'title') else 'Unknown'}")
    ...
    def click_ok_button(self, popup_window):
        ...
        try:
            ...
            popup_window.activate()
            time.sleep(0.5)  # Increased timeout to ensure window is fully active and rendered
